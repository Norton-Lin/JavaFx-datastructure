 ...
 * 数据库需求  :1.闹钟对象化 2.用户登录启动系统时间，同时检索该用户的所有闹钟
 */
public class SystemTime_demo1 {
    ...
    public static class Clock extends JFrame {
        ...
        class Task extends TimerTask {
            ...
            public void run() {
                ...
                //Calendar initial_time = Calendar.getInstance();

                SimpleDateFormat df = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
                String i_t = "2022-05-06 12:21:11";
                ...
                //时钟显示内容
                timeStr = "Current Time: " + year + "年" + month + "月" + date + "日 " + "星期" + week + " " + hour + " : " + minute + " : " + second;
    ...
    public static void main(String[] args) {
        ...
        SystemTime_demo1 test_1 = new SystemTime_demo1();
        //SystemTime test_2 = new SystemTime();

        ArrayList<eventClock> allClock = new ArrayList<>();

        ArrayList<Event> allEvent = new ArrayList<>();
        ArrayList<Integer> allType = new ArrayList<>();


        Time time = new Time();

        time.setStartMonth(5);//活动时间对应的月份
        time.setStartDate(11);//活动时间对应的日期
        time.setWeek(3);//活动时间对应的星期
        time.setStartHour(1);//活动时间对应的小时
        time.setStartMinute(22);//活动时间对应的分钟
        ...
        time_2.setStartHour(6);//活动时间对应的小时
        time_2.setStartMinute(19);//活动时间对应的分钟

        Event event = new Event();
        event.setM_tTime(time);

        Event event_2 = new Event();
        event_2.setM_tTime(time_2);
        ...
        //test_2.setSpeed(600);
        test_1.setClock(event_2, 1);//设置一个闹钟
...
class eventClock {
    ...
    public eventClock(Event event, int type) {
        this.event = event;
        this.type = type;
    }
}
